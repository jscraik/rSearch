...
if [[ -n "$dirty_artifacts" ]]; then
	...
	echo "$dirty_artifacts"
	echo "Commit, stash, or discard those edits before running the freshness check."
	exit 1
fi
...
git -C "$REPO_ROOT" restore --worktree -- "${TRACKED_ARTIFACT_PATHS[@]}"
